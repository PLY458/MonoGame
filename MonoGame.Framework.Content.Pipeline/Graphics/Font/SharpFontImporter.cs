	...
	internal class SharpFontImporter : IFontImporter
	{
		...
		public void Import(FontDescription options, string fontName)
		{
			...
			try
            {
					...
					LineSpacing = face.Size.Metrics.Height.ToInt32();

					// The height used to calculate the Y offset for each character.
					YOffsetMin = -face.Size.Metrics.Ascender.ToInt32();
			}
            finally
            {
				if (face != null)
		...
		private Glyph ImportGlyph(char character, Face face)
		{
            ...
            if (glyphBitmap == null) 
			{
				var gHA = face.Glyph.Metrics.HorizontalAdvance.ToInt32();
				var gVA = face.Size.Metrics.Height.ToInt32();
			...
			abc.A = face.Glyph.Metrics.HorizontalBearingX.ToInt32();
			abc.B = face.Glyph.Metrics.Width.ToInt32();
			abc.C = (face.Glyph.Metrics.HorizontalAdvance.ToInt32()) - (abc.A + abc.B);

			// Construct the output Glyph object.
			return new Glyph(character, glyphBitmap)
			{
				XOffset = -(face.Glyph.Advance.X.ToInt32()),
				XAdvance = face.Glyph.Metrics.HorizontalAdvance.ToInt32(),
				YOffset = -(face.Glyph.Metrics.HorizontalBearingY.ToInt32()),
				CharacterWidths = abc
			};
		}
